...
const SignInForm = () => {
  ...
  const handleSocialLogin = async (user) => {
    try {
      const response = await dispatch(
        usersThunk.facebooklogin(user?._token?.accessToken)
      ).unwrap();
      storage.setStorageJson(USER_LOGIN, response);
      storage.setStorageJson(ACCESS_TOKEN, response.accessToken);
      storage.setCookieJson(USER_LOGIN, response, 30);
  ...
  return (
    <div className={styles.wrapper} style={{ paddingTop: "20px" }}>
      <form onSubmit={handleSubmit}>
        ...
        <div className={styles.flex} style={{ paddingTop: "20px" }}>
          <Space>
            ...
            >
              <Space>
                ...
                <Spin spinning={isLoadingUsers} style={{ color: "#fff" }} />
              </Space>
            </SocialButton>
          </Space>
        </div>
